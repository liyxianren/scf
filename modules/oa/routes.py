...
def api_schedule_progress():
    """纯基于课表数据计算每门课的进度，标记最后3节为即将结课"""
    all_schedules = CourseSchedule.query.order_by(
        CourseSchedule.date, CourseSchedule.time_start).all()

    # 按 (course_name, teacher, students) 分组
    from collections import defaultdict
    groups = defaultdict(list)
    for s in all_schedules:
        key = (s.course_name, s.teacher, s.students or '')
        groups[key].append(s)
    ...
    for key, schedules in groups.items():
        ...
        for idx, s in enumerate(schedules):
            session_num = idx + 1
            is_ending = session_num > max(total - 3, 0)
            progress_map[s.id] = {
                'current': session_num,
                'total': total,
                'is_ending': is_ending,
