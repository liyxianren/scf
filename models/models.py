...
class ProjectPlan(db.Model):
    ...
    slogan = db.Column(db.String(200))  # 一句话描述
    content = db.Column(db.Text)  # 完整 Markdown 计划书
    status = db.Column(db.String(20), default='pending')  # pending, generating, completed, failed
    is_favorited = db.Column(db.Boolean, default=False)  # 收藏标记
    error_message = db.Column(db.Text)  # 失败原因
    source_project_data = db.Column(db.Text)  # 原始项目 JSON (用于重新生成)
    request_context = db.Column(db.Text)  # 请求上下文 JSON (keywords, profile 等)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    completed_at = db.Column(db.DateTime)
    expires_at = db.Column(db.DateTime)  # 3天后过期，收藏后设为 None
